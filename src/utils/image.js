  ...
  return new Promise(res => {
    plus.io.resolveLocalFileSystemURL(src, entry => {
      entry.file(async file => {
        let { width, height } = await getImageInfo(src)
        let scale
        if (width > height && height > 1201) {
          scale = width / height
          height = 1200
          width = 1200 * scale
        } else if (height > width && width > 1201) {
          scale = height / width
          width = 1200
          height = 1200 * scale
        }
        ...
        plus.zip.compressImage(
          {
            src: src,
            dst: src
              .replace('.png', '.compress.png')
              .replace('.PNG', '.compress.PNG')
              .replace('.jpg', '.compress.jpg')
              .replace('.JPG', '.compress.JPG'),
            width: width,
            height: height,
            quality: file.size > 1024 ? 45 : 100,
            overwrite: true
          },
          event => {
            res(event.target)
