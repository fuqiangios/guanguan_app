    ...
    ;(async function () {
      if (
        !(await callWithNext(
          beforeEachFn,
          to,
          ['app-plus', 'app'].indexOf(env) > -1 ? appPlusNowRoute : routerStatus.current
        ))
      ) {
        payload.fail({
...
function wrapNativeFun(nativeFunName) {
  ...
  uni[nativeFunName] = payload => {
    if (payload && payload.hasOwnProperty('name')) {
      let { name } = payload
      let query = ''
      if (name.includes('?')) {
      ...
      const page = uni.pages[name] || null
      if (!page) {
        return
      }
      payload.url = `/${page.path}${query}`
    }
