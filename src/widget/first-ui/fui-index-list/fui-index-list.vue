<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号： 13 0  29 45   982  1，身份证尾号： 2 9067   0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <view class="fui-index__list" ref="fui_index_list" id="fui_index_list">
    ...
    <list class="fui-index__list-sv" scrollable="true" show-scrollbar="false">
      ...
      <template v-for="(list, idx) in lists">
        <header :ref="'fui_il_letter_' + idx">
          <view
            class="fui-index__list-letter"
            :class="{ 'fui-il__key-bg': !background }"
    ...
    <!-- #endif -->
    <view
      class="fui-il__indicator"
      :class="{ 'fui-il__nvue-android': nvueAndroid }"
      :style="{ top: indicators[touchmoveIndex] + 'px' }"
      v-if="touching && touchmoveIndex !== -1"
    >
    ...
    >
      <view class="fui-letter__item" v-for="(item, i) in lists" :key="i">
        <!-- @tap="letterTap(i)" -->
        <text
          class="fui-letter__key"
          :class="{ 'fui-il__key-color': i === touchmoveIndex && !activeBackground }"
          :style="{
            background: i === touchmoveIndex ? activeBackground : 'transparent',
            color: i === touchmoveIndex ? activeColor : keyColor
          }"
          >{{ item.letter }}</text
        >
      </view>
