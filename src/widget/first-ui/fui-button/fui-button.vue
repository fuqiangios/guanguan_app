<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：130  2  9 45  9 8 2 1，身份证尾号： 290   67 0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <view
  ...
  >
    <button
      ...
      :style="{
        ...
        borderWidth: !borderColor || !_borderColor || !isNvue ? '0' : borderWidth,
        ...
        borderRadius: getRadius,
        fontSize: getSize,
        color: _textColor ? _textColor : getColor
      }"
      :loading="loading"
      :form-type="formType"
      :open-type="openType"
    ...
    >
      ...
      <text
        ...
        :class="{
          'fui-btn__gray-color': !background && !disabledBackground && !plain && type === 'gray' && color === '#fff',
          'fui-text__bold': bold
        }"
        v-if="text"
        :style="{
    ...
    <!-- #ifndef APP-NVUE -->
    <view
      v-if="_borderColor || borderColor"
      class="fui-button__thin-border"
      :class="[
        time && (plain || type === 'link') && !disabled ? 'fui-button__opacity' : '',
        disabled && !disabledBackground ? 'fui-button__opacity' : ''
      ]"
      :style="{
...
export default {
  ...
  created() {
    ...
    this.pc = this.isPC()
    // #endif
    if (this.plain && this.type) {
      this._borderColor = `var(--fui-color-${this.type})`
      this._textColor = `var(--fui-color-${this.type})`
    }
  },
  methods: {
    handleStart(e) {
...
.fui-button__active-pc:active::after {
  content: ' ';
  background-color: var(--fui-bg-color-hover, var(--fui-bg-color-hover));
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0;
