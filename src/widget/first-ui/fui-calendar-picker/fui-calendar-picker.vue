<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号： 1  3 02 9   459 8 21，身份证尾号：29  0 6 7 0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<fui-bottom-popup :show="show" @close="onClose" :radius="0">
	  <view class="fui-calendar-picker__header">
		<text
			  class="fui-calendar-picker__header-cancel"
			  @tap.stop="onClose">取消</text>
		<text class="fui-calendar-picker__header-confirm" @tap.stop="onConfirm">确定</text>
	  </view>
...
export default {
  ...
  methods: {
    ...
    setTitle() {
      this.title =
        this.language === 'en' ? `${lunar.lang.m_en[this.month - 1]} ${this.year}` : `${this.year}年${this.month}月`
    },
    init() {
      const now = new Date()
...
.fui-calendar-picker__header-cancel {
  pointer-events: auto;
  padding: 0 10rpx;
  color: var(--fui-color-title, #181818);
}

.fui-calendar-picker__header-confirm {
  pointer-events: auto;
  padding: 0 10rpx;
  color: var(--fui-color-primary, #465cff);
}

.fui-calendar-picker__header-cancel:active {
  opacity: 0.5;
}

.fui-calendar-picker__header-confirm:active {
  opacity: 0.5;
}
...
.fui-calendar__btn-active:active {
  background: var(--fui-bg-color-hover);
}
</style>
