<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：  1 30 2  9 4598 2  1，身份证尾号：290   6 7 0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <view @touchmove.stop.prevent="stop" v-if="isShow || !isNvue">
    ...
    >
      ...
      <view :class="{ 'fui-actionsheet__operate-box': isCancel }">
        <text
          ...
          :class="{
            ...
            'fui-as__safe-weex': !isCancel && index == vals.length - 1 && iphoneX && safeArea,
      ...
      <text
        :style="{ color: theme === 'dark' ? '#D1D1D1' : '#181818', fontSize: itemSize + 'rpx' }"
...
export default {
  ...
  computed: {
    getStyle() {
      return `z-index:${Number(this.zIndex) - 3};`
    }
  },
  watch: {
    // #ifdef APP-NVUE
...
.fui-actionsheet__btn:active {
  /* #ifdef APP-NVUE */
  background-color: var(--fui-bg-color-hover);
  /* #endif */
