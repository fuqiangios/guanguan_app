// 本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：1 30  29   4 598  2 1，身份证尾号：2 9   067 0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。
/**
 * 表单验证
...
const form = {
        ...
        for (let i = 0; i < ruleLen; i++) {
          ...
          switch (ruleItem) {
            case 'required':
              ...
              break
            case 'isPassword':
              isError = !form.isPassword(formData[key])
              break
            case 'isMobile':
  ...
  isPassword: function (value) {
    return /^(?=.*[a-zA-Z])(?=.*[0-9])[A-Za-z0-9]{6,20}$/g.test(value)
  }
}
