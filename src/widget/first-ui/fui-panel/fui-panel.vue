<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：1   3    029  45982 1，身份证尾号： 290    670）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <view
  ...
  >
    ...
    >
      ...
      <view
        v-if="hdBorder"
        :style="{ background: borderColor, left: hdLeft + 'rpx', right: hdRight + 'rpx' }"
    ...
    <view
      ...
      :class="{ 'fui-mediabox__center': !flexStart, 'fui-panel__hover-weex': highlight }"
    ...
    >
      ...
      <view class="fui-panel__mb-hd" v-else>
        <slot name="thumb" />
      </view>
      <view class="fui-panel__mb-bd">
        <text
          ...
          :style="{ color: color, fontSize: size + 'rpx', fontWeight: fontWeight }"
          >{{ item[title] || '' }}</text
        >
        <text
          class="fui-panel__mb-desc"
          v-if="item[desc] || $slots.desc"
...
<style scoped>
.fui-panel__wrap {
  @apply rounded-xl paint-contain;
...
/* #ifndef APP-NVUE */
.fui-panel__hover {
  background-color: var(--fui-bg-color-hover, var(--fui-bg-color-hover)) !important;
}
...
.fui-panel__hover-weex:active {
  background: var(--fui-bg-color-hover) !important;
}
