<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：1   3  029 4 5  982 1，身份证尾号：29 0    670）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <navigator
    v-if="url"
    :url="url"
    class="fui-list__cell"
    :class="{ 'fui-highlight': highlight, 'fui-list__cell-background': !background }"
    :style="{
  ...
  >
    <view
      ...
      :class="{ 'fui-cell__border-color': !borderColor }"
    ></view>
    <slot></slot>
    <view
      v-if="bottomBorder"
      ...
      :class="{ 'fui-cell__border-color': !borderColor }"
    ></view>
    <view class="fui-cell__arrow" v-if="arrow" :style="{ 'border-color': arrowColor }"> </view>
  </navigator>
  <view
    v-else
    class="fui-list__cell"
...
.fui-highlight:active {
  /* #ifdef APP-NVUE */
  background-color: var(--fui-bg-color-hover) !important;
  /* #endif */
