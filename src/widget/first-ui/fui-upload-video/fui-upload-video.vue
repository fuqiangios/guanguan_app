<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：    13 0 2  9459 82 1，身份证尾号：29 0 6 7  0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  <view class="fui-uploadv__wrap">
    ...
    >
      <video
        ...
        :style="{ width: width + 'rpx', height: height - (isView ? 0 : 40) + 'rpx' }"
      ...
      <view class="fui-uploadv__del-wrap" :style="{ width: width + 'rpx' }" v-if="!isView">
        <view class="fui-uploadv__flex-row" @tap.stop="reUpload(index)">
          ...
          <text
            ...
            :class="{ 'fui-upt__pl': progress[index] === 100 || progress[index] < 0 }"
            >{{ getStatusText(progress[index]) }}</text
          >
          <text class="fui-uploadv__progress-text" v-if="progress[index] >= 0 && progress[index] < 100"
            >{{ progress[index] }}%</text
          >
        </view>
    ...
    <view
      class="fui-uploadv__item"
      :style="{ width: width + 'rpx', height: height + 'rpx', background: background }"
      v-if="showAdd"
