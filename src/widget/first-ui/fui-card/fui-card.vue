<!--本文件由FirstUI授权予江苏伟岸纵横科技股份有限公司（手机号：1  30  2 9 4 59  8 21，身份证尾号：2 90 6 7  0）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
  ...
  >
    <view
      ...
      v-if="tag || title || src || $slots.header"
      ...
      <template v-if="!$slots.header">
        <view class="fui-card__header-left">
          <image
            ...
            mode="widthFix"
            v-if="src"
            :style="{ height: height + 'rpx', width: width + 'rpx', borderRadius: imageRadius }"
          ></image>
          <text class="fui-card__header-title" :style="{ fontSize: size + 'rpx', color: color }" v-if="title">{{
            title
          }}</text>
        </view>
