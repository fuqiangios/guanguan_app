<template>
  <view :class="{ 'g-loading__mask': (isMask || isNvue) && isFixed }">
    <view
      class="g-loading g-loading__wrap"
      :class="{ 'g-loading__fixed': isFixed && !isNvue }"
      :style="{ backgroundColor: backgroundColor, position: isFixed && !isNvue ? 'fixed' : 'static' }"
      v-if="type === 'col'"
    >
      <image ref="g_loading" class="g-loading__ani" :src="srcCol"></image>
      <text class="g-loading__text" :style="{ color: colColor, fontSize: size + 'rpx', 'line-height': size + 'rpx' }">{{
        text
      }}</text>
    </view>
    <view
      v-else
      class="g-loading g-loading__row"
      :class="{ 'g-loading__fixed': isFixed && !isNvue }"
      :style="{ position: isFixed && !isNvue ? 'fixed' : 'static' }"
    >
      <image ref="g_loading" class="g-loading-row__ani" :src="srcRow"></image>
      <text class="g-loading__text" :style="{ color: rowColor, fontSize: size + 'rpx', 'line-height': size + 'rpx' }">{{
        text
      }}</text>
    </view>
  </view>
...
export default {
  ...
  methods: {
    // #ifdef APP-NVUE
    _animation() {
      if (!this.$refs['g_loading'] || this.stop) return
      animation.transition(
        this.$refs['g_loading'].ref,
        {
          styles: {
